...
void update_chunk(CHUNK* ch) {
    ...
    *(ch->data_size) = 0;
            ...
            for (int lvl = 0; lvl < MAX_LEVEL + 1; ++lvl) {
                for (int tt = 0; tt < 5; ++tt) {
                    ...
                    for (int j = 0; j < cho[i]->fullBlocks[lvl][tt].size(); j++) {
                        ch->data[it] = cho[i]->fullBlocks[lvl][tt][j]->x + x * CHUNK_SIZE;
                        it++;
                        ch->data[it] = cho[i]->fullBlocks[lvl][tt][j]->y + i * CHUNK_SIZE;
                        it++;
                        ch->data[it] = cho[i]->fullBlocks[lvl][tt][j]->z + y * CHUNK_SIZE;
                        it++;
                        ch->data[it] = 1 << (MAX_LEVEL - cho[i]->fullBlocks[lvl][tt][j]->level);
                        it++;
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float) * it, ch->data);
    //cout << ch->size << " \n";
}
...
void setVAO(int x, int y) {
    ...
    *(ch->data_size) = sum;
    ...
    float test_data[6];
    test_data[0] = ch->data[0];
    test_data[1] = ch->data[1];
    test_data[2] = ch->data[2];
    test_data[3] = ch->data[3];
    test_data[4] = 4.0;
    
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float)*5, &test_data);
