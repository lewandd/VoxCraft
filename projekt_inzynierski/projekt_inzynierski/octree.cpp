...
Octree::Octree(CHUNK* chunk_, int x_, int y_, int o_) {
    this->x = x_;
    this->y = y_;
    this->o = o_;
    this->chunk = chunk_;
...
void Octree::addMinMap(float*** minMap, int lvl, int x, int y, int z, int type, int z0) {
    if (lvl >= 0) {
...
void Octree::add(int x, int y, int z, int type) {
    if (root == NULL)
...
void Octree::setFullBlock(int x, int y, int z, int type, int target_level, bool v0, bool v1, bool v2, bool v3, bool v4, bool v5) {
    if (root == NULL)
        root = new Block(0, 0, 0, 0, 0);
...
int Octree::remove(int x, int y, int z) {
    int last_type = -1;
...
void Octree::show(vector<Block*> v) {
    for (int i = 0; i < (int)v.size(); i++)
        show(v[i]);
}

void Octree::show_all() {
    cout << "----- show_all -----\n\n";
    printf("                lvl |    position     | dim size |               range               |   size    |  max_size  |  type |\n");
    show(root);
}

void Octree::deleteAll() {
    deleteRec(root);
}

void Octree::deleteRec(Block* tmp) {
    if (tmp != NULL) {
        for (int i = 0; i < 8; ++i) {
            deleteRec(tmp->ch[i]);
        }
...
void Octree::addToFullBlocks(Block* n) {
    for (int j = 0; j < 6; ++j) {
        CHUNK* neighch;
        Octree* neigho;
        int nchx, nchy;
...
void Octree::merge(int x, int y, int z) {
    Block* target = root;
    Block* tmp = root;
...
void Octree::deleteFromFullBlocks(Block* n, bool checkNeigh) {
    int bsize = 1 << (MAX_LEVEL - n->level);
...
vector <Block*> Octree::getBlocks(int x, int y, int z) {
